...
<body  onload="load()">
         ...
         <h5 style="display: inline-block; padding-Left:7%; color: white">
           <a style="text-decoration:none; cursor:pointer;" href="index.php" >Home</a> / Search Result
         </h5>
         </div>
...
if($GenType == "All")
 $query = $conn->query("SELECT * FROM movie where movie_name LIKE '%$SearchText%'");
 else
 $query = $conn->query("SELECT * FROM movie where movie_name LIKE '%$SearchText%' AND genre = '$GenType' ");
 ...
 <div class="flip-card" id="flip-card" data-value="<?php echo $row["ID"]; ?>">

     <div class="flip-card-inner">
       <div class="flip-card-front">
        <?php echo "<img style=\"width:175px;height:200px;\" src='".$row['movie_img']."'>"; ?>
      ...
      <?php echo "
         ...
         <button style =\"Left:5%;\" onclick=\"cart('".$row['ID']."','".$row['movie_name']."','".$row['price']."','1')\" class=\"btns\"><i class=\"fas fa-cart-plus\"></i></button>
         <button style =\"Right:5%;\"  class=\"btns\" onclick=\"window.location='MovieDetails.php?Movie_id='+'".$row['ID']."'\" id=\"details\"><i class=\"fas fa-info\"></i></button>
